...
function createOrder(userId,plan){if(plan!=="monthly")throw new Error("套餐不存在");const order={id:crypto.randomBytes(8).toString("hex").toUpperCase(),userId,plan,amount:990,status:"pending",createdAt:Date.now()};db.orders.push(order);save();return order}
